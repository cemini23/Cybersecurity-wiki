...
|------|------|-----------------|
| 2026-08-14 | Full ingest K278–K281 (ATOBench deceptive-verification eval / MARC deterministic orchestration / YAVIN TEE-PIM) + ente Atto steal + OOD MLLM-routed 13463 → image-gen; MARC GO clone 20MB (MIT); ente NO clone (AGPL 704MB); prod+atto+GW+poker briefs | Inbox cleared; verification-chain eval (activity ≠ verification), stage-wise failure attribution, TEE-trust-boundary framing, E2EE consumer-cloud Atto fit. Follow-ups: re-check ATOBench GitHub for real artifacts; MARC pattern only, clinical wont_wire |

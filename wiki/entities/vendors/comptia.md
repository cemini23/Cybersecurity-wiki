---
title: CompTIA
type: entity
tags: [vendor, certifications, vendor-neutral]
keywords: [comptia, security+, pentest+, cysa+, casp+]
related:
  - entities/certifications/comptia-security-plus.md
  - entities/certifications/comptia-pentest-plus.md
  - concepts/cybersecurity-careers.md
  - entities/people/joas-a-santos.md
maturity: draft
created: 2026-05-12
updated: 2026-05-12
---

## Relations

- @entities/certifications/comptia-security-plus.md
- @entities/certifications/comptia-pentest-plus.md
- @concepts/cybersecurity-careers.md
- @entities/people/joas-a-santos.md

## Raw Concept

Cited via Security+ + PenTest+ corpus PDFs.

## Narrative

US-based non-profit IT certification body. Vendor-neutral certs span IT generalist (A+ / Network+) → security (Security+ / PenTest+ / CySA+ / CASP+). Many CompTIA certs are DoD 8570 approved, driving heavy adoption in US federal + defense hiring. [CONFIRMED]

---
title: KAMR knowledge-aligned multi-hop retrieval
type: concept
tags: [concept, rag, knowledge-graph, grounding]
keywords: [KAMR, anchor triplets, GRAG, 2607.27136]
related:
  - sources/arxiv-2607-27136-kamr-multihop-retrieval.md
  - concepts/ai-for-cybersecurity.md
  - concepts/evidence-aware-long-context-grounding.md
  - concepts/tca-sir-target-conditioned-inspiration-retrieval.md
  - sources/arxiv-2607-28498-tca-sir-scientific-inspiration.md
maturity: draft
created: 2026-07-30
updated: 2026-07-31
---

## Relations

- @sources/arxiv-2607-27136-kamr-multihop-retrieval.md
- @concepts/ai-for-cybersecurity.md
- @concepts/evidence-aware-long-context-grounding.md
- @concepts/tca-sir-target-conditioned-inspiration-retrieval.md
- @sources/arxiv-2607-28498-tca-sir-scientific-inspiration.md

## Raw Concept

Multi-hop KG retrieval needs query-aligned anchors plus structurally connected weak matches — not global semantic rank alone.

## Narrative

Pattern for threat-intel and archival KGs. No LICENSE on public code → pattern-only until cleared. [CONFIRMED abstract]

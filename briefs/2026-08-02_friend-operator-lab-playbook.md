...
- `@concepts/rtl-codegen-poison-defense.md` · `@concepts/trace-verified-ctf-agent-eval.md` · `@concepts/non-decaying-loop-safety-state.md` · `@concepts/experience-driven-redteam-skill-evolution.md` (K310–K313; K312 runtime hook 2026-08-28)
...
36. **K312 loop-state hook is live (2026-08-28)** — this wiki now chains `python3 scripts/k312_loop_state.py --hook` after K303 on `beforeShellExecution` / `preToolUse`. Unauthorized irreversible actions (force-push, `curl|sh`, `ssh cemini-prod`, Docker `--network=host`, LIVE Discord, `mcp.json` / `.cursor/skills` writes) hit a bound (default 3) then deny until `python3 scripts/k312_loop_state.py grant --n N --reason "…"`. Grant does **not** zero history. Known mediated paths (`git push` without `--force`, archive-to-egress, prod briefs scp) are logged and allowed. Do not clone `getathelas/LoopHarness`. StepGuard (`zheng977/StepGuard`) still **no LICENSE** — `bash scripts/stepguard_inventory.sh check`; no clone / no HF weights.

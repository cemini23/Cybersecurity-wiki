...
PY = [sys.executable, str(ROOT / "scripts/k322_firmware_rehost_precheck.py")]
KEYS = (
    "owned_device_scope",
    "fidelity_criterion",
    "peripheral_modeling",
    "emulator_not_device",
    "no_wiki_exploit_payloads",
)
...
def test_json_fail() -> None:
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump({k: True for k in KEYS} | {"fidelity_criterion": False}, f)
        path = f.name
    r = run("json", "--json", path)
...
if __name__ == "__main__":
    ...
    test_json_ok()
    test_json_fail()
    print("OK test_k322_firmware_rehost_precheck")

...
PY = [sys.executable, str(ROOT / "scripts/k328_rag_safety_bench_precheck.py")]
KEYS = (
    "corpus_conditions", "base_vs_rag", "judge_named",
    "pairs_k323", "no_scalar_certificate",
)
...
def test_json_fail() -> None:
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump({k: True for k in KEYS} | {"pairs_k323": False}, f)
        path = f.name
...
if __name__ == "__main__":
    ...
    test_json_ok()
    test_json_fail()
    print("OK test_k328_rag_safety_bench_precheck")

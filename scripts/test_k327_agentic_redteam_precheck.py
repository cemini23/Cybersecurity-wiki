...
PY = [sys.executable, str(ROOT / "scripts/k327_agentic_redteam_precheck.py")]
KEYS = (
    "written_scope", "taxonomy_domains", "multi_step_harness",
    "verification_mode", "no_wiki_payloads",
)
...
def test_json_fail() -> None:
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump({k: True for k in KEYS} | {"verification_mode": False}, f)
        path = f.name
...
if __name__ == "__main__":
    ...
    test_json_ok()
    test_json_fail()
    print("OK test_k327_agentic_redteam_precheck")

...
PY = [sys.executable, str(ROOT / "scripts/k347_asleval_precheck.py")]
KEYS = (
    "written_scope", "session_level", "target_preregistered",
    "displacement_named", "no_wiki_payloads",
)
...
def test_json_fail() -> None:
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump({k: True for k in KEYS} | {"displacement_named": False}, f)
        path = f.name
...
if __name__ == "__main__":
    ...
    test_json_ok()
    test_json_fail()
    print("OK test_k347_asleval_precheck")

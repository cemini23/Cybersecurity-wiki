...
PY = [sys.executable, str(ROOT / "scripts/k352_pir_sandbagging_precheck.py")]
KEYS = (
    "written_scope", "reference_free_claim", "concealment_vs_erasure",
    "counterfactual_pair", "no_wiki_payloads",
)
...
def test_json_fail() -> None:
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump({k: True for k in KEYS} | {"no_wiki_payloads": False}, f)
        path = f.name
...
if __name__ == "__main__":
    ...
    test_json_ok()
    test_json_fail()
    print("OK test_k352_pir_sandbagging_precheck")

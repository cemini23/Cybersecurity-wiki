...
PY = [sys.executable, str(ROOT / "scripts/k331_privescalate_precheck.py")]
KEYS = (
    "written_scope", "scenario_count", "executable_verify",
    "harness_named", "no_wiki_exploits",
)
...
def test_json_fail() -> None:
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump({k: True for k in KEYS} | {"executable_verify": False}, f)
        path = f.name
...
if __name__ == "__main__":
    ...
    test_json_ok()
    test_json_fail()
    print("OK test_k331_privescalate_precheck")
